...
| Query history | 14 messages; 2,000 chars/message; 12,000 chars total | Firestore rule permits 40 stored messages/document | UNVERIFIED under full route | stored history can reach Firestore document-size limit | 14 sent; 40 stored messages | CODE_VERIFIED constants/rules |
| Files/user | Firestore tier config; fallback FREE=5, PRO=50, UNLIMITED=9,999 | UNVERIFIED | UNVERIFIED | remote config may differ | 5 personal PDFs | CODE_VERIFIED fallback; PROPOSED |
| File size | fallback FREE=10 MB, PRO=50 MB | UNVERIFIED | UNVERIFIED | full upload buffered in memory | 10 MB | CODE_VERIFIED tier service; PROPOSED |
| Extracted text | 500,000 chars/document for constrained tiers; `UNLIMITED` bypasses the cap | UNVERIFIED | UNVERIFIED | parser path unmeasured | monitor; retain pending hosted evidence | CODE_VERIFIED |
| Chunks/document | 1,500 for constrained tiers; `UNLIMITED` bypasses the cap | Chroma local volume unbounded in app | 17 chunks used 315,556 B additional disk | storage grows without global quota | monitor; retain pending hosted evidence | CODE_VERIFIED limit; MEASURED_LOCAL artifact |
