...
def main():
    ...
    text = (
        "Synthetic capacity document: renewal terms, access controls, and retention policy. "
        * 90
    )
    chunks = [
        Document(
            page_content=text[i : i + 512],
            metadata={
                "source": tenant,
        ...
        for n, i in enumerate(range(0, len(text), 462))
    ]
    ...
    def query(_: int):
        ...
        docs = repository.similarity_search(
            "What is the retention policy?", tenant, k=3
        )
        time.sleep(0.08)
    ...
    for concurrency in (1, 2, 5, 10, 20):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=concurrency) as executor:
            ...
            for future in futures:
                try:
                    latencies.append(future.result(timeout=30)[0])
                except Exception:
                    errors += 1
        ...
        rows.append(
            {
                ...
                "errors": errors,
                "error_rate": errors / (concurrency * 5),
                ...
                "cpu_seconds": (process.cpu_times().user - cpu_before.user)
                + (process.cpu_times().system - cpu_before.system),
            }
        )
    result = {
        "run_at": datetime.now(timezone.utc).isoformat(),
        "scope": "MEASURED_LOCAL: isolated direct repository/retrieval benchmark; OpenAI final-answer stage mocked at 80ms. It does not measure HTTP routing, Firestore, PDF parser, Railway, or OpenAI.",
        "machine": {
    ...
    (ARTIFACTS / f"local-benchmark-{stamp}.json").write_text(
    ...
    print(
        json.dumps(
            {
                "artifact": str(ARTIFACTS / f"local-benchmark-{stamp}.json"),
                "result": result,
            },
            indent=2,
        )
