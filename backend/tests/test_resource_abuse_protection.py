...
def test_unlimited_tier_uses_its_configured_upload_capacity(
    ...
    """UNLIMITED accounts must not inherit the public FREE upload limit."""
    monkeypatch.setattr(
    ...
    assert tier_limit_service.get_max_upload_size_bytes("owner") == 9999 * 1024 * 1024
    assert tier_limit_service.check_file_count_limit("owner", 9998) == (True, 9999)
    assert tier_limit_service.check_file_count_limit("owner", 9999) == (False, 9999)
