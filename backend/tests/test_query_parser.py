...
def _new_parser() -> QueryParserService:
    return QueryParserService(
        llm=MagicMock(spec=BaseChatModel), embeddings=MagicMock(spec=Embeddings)
    )
...
class TestFileFilterExtraction:
    ...
    def test_uses_the_injected_llm(self) -> None:
        """The parser uses the model selected by its composition root."""
        llm = MagicMock(spec=BaseChatModel)
        service = QueryParserService(llm=llm, embeddings=MagicMock(spec=Embeddings))
    ...
    def test_no_filters_found(self, mock_str_parser_invoke: Any) -> None:
        ...
        # Test extraction
        result = service.extract_file_filters(
            query="What is the main topic?", available_files=["doc1.pdf", "doc2.pdf"]
        )
    ...
    @patch.object(StrOutputParser, "invoke")
    def test_validates_against_available_files(self, mock_str_parser_invoke: Any) -> None:
    ...
    def test_handles_llm_failure_gracefully(self) -> None:
        ...
        service = QueryParserService(
            llm=mock_llm_instance, embeddings=MagicMock(spec=Embeddings)
        )

        # Should not crash, should return fallback (no filters)
        result = service.extract_file_filters(
            query="Cerca nel file report.pdf", available_files=["report.pdf"]
        )
        ...
        assert len(result.exclude_files) == 0
        assert (
            result.cleaned_query == "Cerca nel file report.pdf"
...
class TestCaseInsensitiveMatching:
    ...
    def test_case_insensitive_exclude(self, mock_str_parser_invoke: Any) -> None:
        ...
        result = service.extract_file_filters(
            query="Cerca senza Notes.PDF", available_files=["report.pdf", "notes.pdf"]
        )
...
def test_compound_analysis_returns_at_most_two_retrieval_queries(
    ...
    result = _new_parser().extract_file_filters(
