...
class DocumentMetadata(BaseModel):
    ...
    uploaded_at: str | None = Field(None, description="Upload timestamp")
...
class SourceCitation(BaseModel):
    """A source selected by retrieval/reranking, never generated by the LLM."""

    filename: str = Field(..., min_length=1, description="Original document filename.")
    page_number: int | None = Field(
        default=None,
        ge=1,
        description="One-based PDF page number when the retrieved chunk has one.",
    )
...
class QueryResponse(BaseModel):
    ...
    citations: list[SourceCitation] = Field(
        default_factory=list,
        description="Retrieved source citations with optional page metadata.",
    )
