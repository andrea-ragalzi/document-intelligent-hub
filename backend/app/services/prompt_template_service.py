...
class PromptTemplateService:
    ...
    def build_modular_prompt(
        ...
        section_iv = self._build_context_section(additional_context) if additional_context else ""
...
{constraint_number}. **FORMAT CONSTRAINT (PRIORITY):** {constraints.format_constraint}
   ...
   - Follow this format structure precisely."""
        ...
        if constraints.additional_constraints:
            for i, additional in enumerate(constraints.additional_constraints, start=constraint_number):
                section += f"""
...
{additional_context}"""
    ...
    def create_constraints_for_use_case(
        ...
        if quantity is not None:
            quantity_constraint = f"The response MUST contain EXACTLY {quantity} items/elements."
        ...
        # Format constraint
        format_constraint = custom_format or f"Output MUST be formatted as: {output_format_desc}"
