...
import { MessageSquare, CornerDownLeft, Loader, Save } from 'lucide-react';
    ...
    return (
        <div className="lg:w-2/3 flex flex-col bg-white dark:bg-gray-800 rounded-2xl shadow-xl transition-colors duration-500">
            <header className="p-5 border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50 rounded-t-2xl flex justify-between items-center">
                ...
                <button
                    onClick={onSaveClick}
                    disabled={chatHistory.length === 0 || isQuerying || !userId}
                    className={`flex items-center py-2 px-3 text-sm font-bold rounded-lg shadow-md transition duration-200 ${chatHistory.length === 0 || isQuerying || !userId
                        ? 'bg-gray-300 text-gray-600 cursor-not-allowed dark:bg-gray-700 dark:text-gray-400'
                        : 'bg-green-500 text-white hover:bg-green-600'
                        }`}
                >
                    <Save size={16} className="mr-2" /> Salva Chat
                </button>
            </header>
            ...
            >
              ...
              bg-white dark:bg-gray-900 dark:border-gray-600 dark:text-white disabled:dark:bg-gray-700"
                    ...
                    <button
                        ...
                        className={`flex items-center justify-center p-3 rounded-lg shadow-md transition duration-200 ${isQuerying || !query.trim() || !userId
