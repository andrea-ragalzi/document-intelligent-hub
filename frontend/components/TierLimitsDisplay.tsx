...
export default function TierLimitsDisplay({
  ...
  const documentPercentage = isUnlimited
    ? 0
    : (currentDocuments / limits.maxDocuments) * 100;
  const queryPercentage = isUnlimited
    ? 0
    : (currentQueries / limits.maxQueriesPerDay) * 100;

  const isDocumentLimitClose = documentPercentage > 80;
  const isQueryLimitClose = queryPercentage > 80;
        ...
        <div className="space-y-3">
            ...
            <div className="flex items-center justify-between text-xs">
              ...
              <span
                className={`font-medium ${
                  isDocumentLimitClose
                    ? "text-orange-600 dark:text-orange-400"
                    : "text-gray-700 dark:text-gray-300"
                }`}
              >
                {currentDocuments} / {limits.maxDocuments}
              </span>
            </div>
            <div className="h-1.5 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full transition-all duration-300 ${
                  isDocumentLimitClose ? "bg-orange-500" : "bg-blue-500"
                }`}
                style={{ width: `${Math.min(documentPercentage, 100)}%` }}
              />
            </div>
          </div>
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between text-xs">
              ...
              >
                {currentQueries} /{" "}
                {limits.maxQueriesPerDay === Infinity
                  ? "∞"
                  : limits.maxQueriesPerDay}
              </span>
            </div>
            <div className="h-1.5 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full transition-all duration-300 ${
                  isQueryLimitClose ? "bg-orange-500" : "bg-green-500"
                }`}
          ...
          {/* Upgrade CTA */}
          {(isDocumentLimitClose || isQueryLimitClose) && (
